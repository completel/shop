        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前正常路由跳转，从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，发请求把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      /* 
        当前这里是派发一个action，也向服务器发请求，判断加入购物车失败了还是成功了，进行相应的操作
        this.$store.dispatch("addOrUpdateShopCart", {
        skuId: this.$route.params.skuid,
        skuNum: this.skuNum,
        });
        2. 上面这行代码说白了：调用仓库中的addOrUpdateShopCart 这个方法加上asyc，返回一定是一个Promise
        要么成功|要么失败
      */
      try {
        // 成功
        ...
        // 3. 进行路由的跳转
        // 4. 在路由跳转的时候还需要将产品的信息带个下一级路由组件
        // 一些简单的参数：skuNum，通过query参数传递过去
        // 产品信息的数据【比较复杂：skuInfo】，通过会话存储（不持久化，会话结束就消失）
        // 本低存储|会话存储，一般存储的都是字符串
