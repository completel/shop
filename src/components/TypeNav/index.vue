...
export default {
  ...
  methods: {
    ...
    goSearch(event) {
      // 最好的解决方案：编程式导航 + 事件委派
      // 利用的事件委派存在一些问题：1：点击一定的是a标签 2：如何获取参数【1、2、3级分类产品的名字 id】
      // 点击a标签的时候，才会进行路由跳转【怎么能确定点击的一定是a标签】
      // 存在的另外一个问题：即使确定点击的是a标签，如何区分是一级、二级、三级标签
  ...
  .container {
    ...
    .sort {
      ...
      .all-sort-list2 {
        ...
        .cur {
          background-color: rgb(242, 38, 72);
        }
      }
    }
    // 过渡动画
